...
export default function DatePicker({ open, value, onChange, onClose }: Props) {
  ...
  const handleConfirm = () => {
    const dateStr = `${tempDate.year}-${padZero(tempDate.month)}-${padZero(tempDate.day)}`;
    onChange(dateStr);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/30">
          ...
          <h3 className="text-base font-semibold text-foreground">{language === 'zh' ? '选择日期' : 'Select Date'}</h3>
          <button
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input
                  type="number"
                  value={tempDate.year}
                  onChange={(e) => setTempDate(prev => ({ ...prev, year: parseInt(e.target.value) || currentYear }))}
